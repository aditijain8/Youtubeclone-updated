...
function VideoPage() {
  ...
  const getWeather = async (lat, lon) => {
    const apiKey = "3e524c8b72d54fb38f6151358242407"; // Replace with your WeatherAPI key
    const response = await fetch(
      `http://api.weatherapi.com/v1/current.json?key=3e524c8b72d54fb38f6151358242407&q=India&aqi=no`
    );
    if (response.ok) {
          ...
          >
            ...
            ref={videoRef}
             // src={`http://localhost:5500/${vv?.filePath}`}
               src={`https://your-tube-clone-4vtc.onrender.com/${vv?.filePath}`}
